const STORAGE_KEY = window.OPFState?.STORAGE_KEY || "opf-financeiro-data-v2";
const THEME_KEY = window.OPFState?.THEME_KEY || "opf-financeiro-theme";

// =========================
// Estado da aplicacao
// =========================

const state = window.OPFState?.state || {
  cadastros: {
    emitentes: [],
    fornecedores: [],
    naturezas: [],
...
const GLOBAL_MODULES = window.OPFState?.GLOBAL_MODULES || [
    {
      name: "WeRecibos",
...
function lerCentavosMoneyInput(input) {
  if (!input) {
    return 0;
  }

  const centsFromDataset = Number(input.dataset.cents);
  if (!Number.isNaN(centsFromDataset) && centsFromDataset > 0) {
    return centsFromDataset;
  }
...
function atualizarResumo() {
  ...
  linhas.forEach((linha) => {
    ...
    const unitario = lerCentavosMoneyInput(linha.querySelector(".item-unitario")) / 100;
...
function salvarRascunhoEFecharFormulario() {
  ...
  if (!documento) {
    mostrarToast("Nao foi possivel salvar a OPF. Revise os dados preenchidos.", "error");
    return;
  }
...
function coletarDocumentoAtual({ validarPix = true } = {}) {
  ...
  const itens = Array.from(document.querySelectorAll("#lancamentoItensBody tr"))
    .map((linha) => {
      ...
      return {
        ...
        __vazio: !codigo && !descricao && quantidade <= 0 && unitario <= 0
      };
    })
    .filter((item) => !item.__vazio)
    .map(({ __vazio, ...item }) => item);

  const subtotal = itens.reduce((acc, item) => acc + item.total, 0);
  const desconto = lerCentavosMoneyInput(document.getElementById("descontoInput")) / 100;
  const acrescimo = lerCentavosMoneyInput(document.getElementById("acrescimoInput")) / 100;
  const total = Math.max(subtotal - desconto + acrescimo, 0);
...
function sincronizarCamposPix() {
  ...
  const possuiTipo = Boolean(pixKeyType?.value);
  ...
  if (!ativo) {
    ...
    if (pixKey) {
      ...
      pixKey.disabled = true;
      pixKey.placeholder = "Selecione primeiro o tipo da chave";
    }
    return;
  }

  if (pixKey) {
    pixKey.disabled = !possuiTipo;
    pixKey.placeholder = possuiTipo ? "Digite a chave PIX" : "Selecione primeiro o tipo da chave";
    if (!possuiTipo) {
      pixKey.value = "";
      pixKey.setCustomValidity("");
    }
...
function popularPreview(documento) {
  ...
  const subtotalCalculado = itens.reduce((acc, item) => acc + Number(item.total || (Number(item.quantidade || 0) * Number(item.unitario || 0))), 0);
  const descontoCalculado = Number(documento.desconto || 0);
  const acrescimoCalculado = Number(documento.acrescimo || 0);
  const totalCalculado = Math.max(subtotalCalculado - descontoCalculado + acrescimoCalculado, 0);
  ...
  document.getElementById("previewSubtotal").textContent = formatarMoeda(subtotalCalculado);
  document.getElementById("previewDesconto").textContent = formatarMoeda(descontoCalculado);
  document.getElementById("previewAcrescimo").textContent = formatarMoeda(acrescimoCalculado);
  document.getElementById("previewTotalDocumento").textContent = formatarMoeda(totalCalculado);
  ...
  document.getElementById("previewItensBody").innerHTML = itens.length ? itens.map((item) => `
    <tr>
      <td>${item.codigo}</td>
      <td>${item.descricao}</td>
  ...
  `).join("") : `
    <tr>
      <td colspan="5" class="center">Nenhum item informado.</td>
    </tr>
  `;
}
...
function abrirPreviewComLoading() {
  ...
  if (!documento) {
    mostrarToast("Nao foi possivel gerar o documento. Revise os campos obrigatorios.", "error");
    return;
  }
